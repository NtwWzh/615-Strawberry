...
1.  Delate all the columns that only has NA value since it is useless, cannot provide useful information for analysis.
2.  In the Value column, there are some data that aren't numbers, which is (D). They will be replaced with 0 first, then replaced by the average of the Value according to the type of data item they belong to.
...
result <- straw %>%
  ...
  group_by(`Data Item`) %>%
  summarize(total_value = round(mean(Value, na.rm = TRUE)))

straw <- straw %>%
  left_join(result, by = 'Data Item') %>%
  mutate(Value = ifelse(Value == "0", total_value, Value))
